<nav class="side-menu">
            
            <ul class="side-menu-list p-0">
                <li class="red">
                    <a href="inicio.php" class="activo">
                        <img src="../public/img-inicio/team.png" class="img-inicio" alt="">
                        <!-- <i class="fas fa-house-user"></i> -->
                        <span class="lbl">EMPLEADOS</span>
                    </a>
                </li>

                <li class="grey with-sub">
                    <span>
                        <img src="../public/img-inicio/programar.png" class="img-inicio" alt="">
                        <!-- <i class="fas fa-sort-amount-up-alt"></i> -->
                        <span class="lbl">ASISTENCIAS</span>
                    </span>
                    <ul>
                        <li>
                            <a href="G5.php" class="">
                                <i class="fas fa-th-list icono-submenu"></i>
                                <span class="lbl">Asistencia G5 PAV</span>
                            </a>
                        </li>
                        <li>
                            <a href="G10.php" class="">
                                <i class="fas fa-th-list icono-submenu"></i>
                                <span class="lbl">Asistencia G10 PAV</span>
                            </a>
                        </li>
                        <li>
                            <a href="G11.php" class="">
                                <i class="fas fa-th-list icono-submenu"></i>
                                <span class="lbl">Asistencia G11 NOPAV</span>
                            </a>
                        </li>
                    </ul>
                </li>

                <li class="red">
                    <a href="maquinaria.php" class="">
                        <img src="../public/img-inicio/maquinaria.png" class="img-inicio" alt="">
                        <!-- <i class="fas fa-exclamation"></i> -->
                        <span class="lbl">EQUIPO</span>
                    </a>
                </li>

                <li class="red">
                    <a href="acerca.php" class="">
                        <img src="../public/img-inicio/info.png" class="img-inicio" alt="">
                        <!-- <i class="fas fa-exclamation"></i> -->
                        <span class="lbl">ACERCA DE </span>
                    </a>
                </li>




            </ul>
        </nav>